...
let exp = module.exports={
        ...
        timeLoop(waitToNextLoop = 100, loops = 1, callback = ()=>{}, callbackEnd = ()=>{}){
            var count = 0;
            var loop = setInterval(()=>{
                count++;
                callback();
                if(count >= loops){clearInterval(loop); callbackEnd()}
            }, waitToNextLoop)
        }
};
...
exp.easyJson = class{
    ...
    async save(){
        fs.writeFileSync(this.path, JSON.stringify(this.json, null, 2), function (err) {
